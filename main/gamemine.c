...
//	memset( Segments, 0, sizeof(segment)*MAX_SEGMENTS );
	...
	for (i=0; i<Num_segments; i++) {
		...
		fuelcen_activate( &Segments[i], Segment2s[i].special );
		//CTFC Begin	
		//One home segment for each team is remembered as flag spawn point
		//this is to speed up the process and avoid searching through all segments
		//during the game
		if (Game_mode && GM_CAPTURE)
		{
			...
			}	else if (Segment2s[i].special == SEGMENT_IS_GOAL_RED)
			{	
				goal_red_cube = i;
		...
		//CTFC End		
		//
		//wtf find flags and fucking move them
		//Segment2s[i].objects
