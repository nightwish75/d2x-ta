...
void multi_compute_kill(int killer, int killed)
{
	...
	if (killer_pnum == killed_pnum)
	{
		...
		{
			//Turkey Begin
			//suicides don't count in Turkey
			if(!(Game_mode & GM_TURKEY)) {
				//Turkey End
				if (Game_mode & GM_TEAM)
				{
	...
	else
	{
		if (!(Game_mode & GM_HOARD))
		{
			if (Game_mode & GM_TEAM)
			{
				...
				{
					//CTFC Begin patching TA Begin
					if ((Game_mode & GM_CAPTURE) || (Game_mode & GM_TA))
...
//TA Begin
//TODO Here handle /timeout /timein commands @Nightwish
//TA End
	else
		HUD_init_message(HM_MULTI, "%s '%s'", TXT_SENDING, Network_message);
...
multi_do_fire(const ubyte *buf)
{
	...
	//TA Begin
	//TODO no hurry with spawning if in TA mode, need to make sure you can spawn and force it elsewhere
	//TA End
