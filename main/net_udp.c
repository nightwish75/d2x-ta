...
typedef struct param_opt
{
	...
	int closed, refuse, maxnet, anarchy, team_anarchy, robot_anarchy, coop, capture, hoard, team_hoard, bounty, arena, turkey;
} param_opt;
...
int net_udp_game_param_handler( newmenu *menu, d_event *event, param_opt *opt )
{
	...
	{
		case EVENT_NEWMENU_CHANGED:
			if (((HoardEquipped() && (citem == opt->team_hoard)) || ((citem == opt->turkey || citem == opt->arena || citem == opt->team_anarchy) || (citem == opt->capture))) && !menus[opt->closed].value && !menus[opt->refuse].value) 
			{
			...
			if ((citem >= opt->mode) && (citem <= opt->mode_end))
			{
				...
				else if( menus[opt->arena].value )
					Netgame.gamemode = NETGAME_TEAM_ARENA;
				else if( menus[opt->turkey].value )
					Netgame.gamemode = NETGAME_TEAM_TURKEY;				
...
//TA Turkey Begin
	m[optnum].type = NM_TYPE_RADIO; m[optnum].text = "Team Arena"; m[optnum].value=(Netgame.gamemode == NETGAME_TEAM_ARENA); m[optnum].group=0; opt.arena=optnum; optnum++;
	m[optnum].type = NM_TYPE_RADIO; m[optnum].text = "Turkey Shoot"; m[optnum].value=(Netgame.gamemode == NETGAME_TEAM_TURKEY); m[optnum].group=0; opt.turkey=optnum; optnum++;
...
net_udp_set_game_mode(int gamemode)
{
	...
	else if( gamemode == NETGAME_TEAM_ARENA )
	{
		Game_mode = GM_NETWORK | GM_TEAM | GM_TA;
		Show_kill_list = 3;
	}
	else if( gamemode == NETGAME_TEAM_TURKEY )
	{
		Game_mode = GM_NETWORK | GM_TEAM | GM_TURKEY;
		Show_kill_list = 3;
		Netgame.BornWithBurner=1;
	}
	else
		Int3();
}
...
net_udp_select_teams(void)
{
	...
	if(Netgame.gamemode == NETGAME_TEAM_TURKEY)
	{
	sprintf(team_names[0], "%s", "Hunters");
	sprintf(team_names[1], "%s", "Turkey");
	}
...
// Let host join without Client available. Let's see if our players like that
	...
	if (Netgame.gamemode == NETGAME_TEAM_ANARCHY ||
	    Netgame.gamemode == NETGAME_CAPTURE_FLAG ||
		 Netgame.gamemode == NETGAME_TEAM_HOARD || Netgame.gamemode == NETGAME_TEAM_ARENA || Netgame.gamemode == NETGAME_TEAM_TURKEY)
